...
// RDD[(Int, List[(Int, Double)])]
// RDD[(BlockNo, List[(Year, AverageAqi)])]
...
object PreprocessAQI extends java.io.Serializable{

  def run(sc: SparkContext, input: String, blockinput: String): RDD[(Int, List[(Int, Double)])] = {
    //val input = "/user/yz3940/bdad/project/data/aqi"
    val emptyoutput = input + "empty"
